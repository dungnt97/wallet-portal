...
describe('requireStepUp middleware', () => {
  ...
  it('returns 401 when staff is not in session', async () => {
    ...
    await handler(req as FastifyRequest, reply as unknown as FastifyReply, vi.fn() as unknown as () => void);
  ...
  it('does not call reply when steppedUpAt is within 5 minutes', async () => {
    ...
    const reply = makeReply();
    const next = vi.fn();
    await handler(req as FastifyRequest, reply as unknown as FastifyReply, next as unknown as () => void);
