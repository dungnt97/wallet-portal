...
try {
  ...
  execFileSync('pnpm', ['dlx', 'openapi-typescript', specUrl, '-o', outFile], { stdio: 'inherit' });
  console.log('\nDone. Commit src/api/types.gen.ts when schema stabilises.');
} catch {
  console.error('\nFailed to generate types. Is admin-api running on :3001?');
  console.error(
    'Start it: DATABASE_URL=postgres://postgres:postgres@localhost:5433/postgres pnpm --filter @wp/admin-api dev'
  );
  process.exit(1);
}
